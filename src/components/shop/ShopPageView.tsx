...
import { CHASSIS_CATEGORY_SLUG, CHASSIS_CATEGORY_HREF } from "@/lib/shop-links";
...
export function findCategoryByParam(categories: ShopCategory[], categoryParam: string) {
  return (
...
export function buildCategoryLookup(categories: ShopCategory[]) {
...
interface ShopPageViewProps {
  ...
  // When the category lives in the URL path (/shop/<slug>), basePath points
  // pagination links there instead of /shop?category=<slug>.
  basePath?: string;
}

export async function ShopPageView({ params, basePath }: ShopPageViewProps) {
  ...
  const isChassisCategory = params.category === CHASSIS_CATEGORY_SLUG ||
    selectedCategory?.slug === CHASSIS_CATEGORY_SLUG;
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8 md:py-12">
      ...
      {/* ── Mobile-only category landing ─────────────────────────────
          ...
          /shop/<slug> which renders the category page with the
